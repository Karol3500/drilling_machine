...
public class AntPanel extends JPanel implements ActionListener {
	...
	JLabel notice;
	JCheckBox statsOn;
	JTextField antCount,alpha,beta,q,pheromonePersistence,initialPheromones;
	...
	public AntPanel() {
		...
		pheromonePersistence = new JTextField("0.3", 5);
		lf_pers = new JLabel("Współczynnik wyparowywania feromonów[0,1]");
		lf_pers.setLabelFor(pheromonePersistence);
		gbcBS.gridx = 0;
		gbcBS.gridy = 4;
		basicSettings.add(pheromonePersistence, gbcBS);
		gbcBS.gridx = 1;
		gbcBS.gridy = 4;
		basicSettings.add(lf_pers, gbcBS);

		initialPheromones = new JTextField("1.2", 5);
		lf_init = new JLabel("Początkowe feromony");
		lf_init.setLabelFor(initialPheromones);
		gbcBS.gridx = 0;
		gbcBS.gridy = 5;
		basicSettings.add(initialPheromones, gbcBS);
		gbcBS.gridx = 1;
		gbcBS.gridy = 5;
